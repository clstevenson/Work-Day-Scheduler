...
$(function () {
  ...
  // returns date as string "MMM-DD" to allow one to determine if header needs updating
  function displayDate() {
    var now = dayjs();
    $('#currentDay').text(now.format("dddd, MMMM D") + "th");
    return now.format("MMM-DD");
  }
  // eventually will be called in a timer; delete the line below when that is added
  displayDate();
}); // end ready function
